...
class QdrantService {
  ...
  async upsertPoints(points) {
    try {
      ...
      console.log(`🔍 Validating ${points.length} points for 384-dimensional vectors...`);
      ...
      console.log(`✅ Successfully upserted ${points.length} points to local embedding collection (384 dimensions)`);
  ...
  async getEmbedding(text, taskDescription = "Generate semantic embeddings for interior design search") {
    try {
      const embedding = await embeddingService.getEmbedding(text, taskDescription);
    ...
    for (const [field, expectedDimension] of Object.entries(expectedDimensions)) {
      if (point.vectors && point.vectors[field]) {
        ...
        if (!Array.isArray(vector) || vector.length !== expectedDimension) {
          throw new Error(`Invalid ${field} vector dimensions: expected ${expectedDimension}, got ${vector?.length || 'undefined'}`);
        }
      }
    }
  }
  ...
  async search(
    ...
    try {
      ...
      const [primarySearchVector, semanticDescVector, objectFocusVector] = 
        await Promise.all([
          this.getEmbedding(enhancedQuery.enhanced_query?.primary_search || query, "Interior design room search"),
          this.getEmbedding(enhancedQuery.enhanced_query?.semantic_desc || query, "Interior design style search"),
          this.getEmbedding(enhancedQuery.enhanced_query?.object_focus || query, "Furniture and object search"),
        ]);
      ...
      console.log(`📊 Search results: Primary: ${primaryResults.length}, Semantic: ${semanticResults.length}, Object: ${objectResults.length}`);
  ...
  async multiVectorSearch(queryEmbeddings, limit = 100, filter = null, weights = { primary_search: 0.4, semantic_desc: 0.3, object_focus: 0.3 }) {
    try {
      const searches = await Promise.all([
        this.client.search(this.collectionName, {
          vector: { name: "primary_search", vector: queryEmbeddings.primary_search },
          limit: limit,
          with_payload: true,
          with_vector: false,
          filter: filter
        ...
        this.client.search(this.collectionName, {
          vector: { name: "semantic_desc", vector: queryEmbeddings.semantic_desc },
          limit: limit,
          with_payload: true,
          with_vector: false,
          filter: filter
        ...
        this.client.search(this.collectionName, {
          vector: { name: "object_focus", vector: queryEmbeddings.object_focus },
          limit: limit,
          with_payload: true,
          with_vector: false,
          filter: filter
  ...
  async recreateCollection() {
    try {
      console.log(`🔄 Force recreating collection '${this.collectionName}' with correct dimensions...`);
