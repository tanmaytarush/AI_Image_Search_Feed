...
class SearchIntelligenceService {
  ...
  async initializeQueryModel() {
    if (!this.initialized) {
      try {
        ...
        console.log(
          "✅ Successfully initialized HF models: all-MiniLM-L6-v2 (384 dims)"
        );
      } catch (error) {
  ...
  async getQueryEmbedding(text) {
    ...
    try {
      ...
      if (embedding.length !== this.expectedEmbeddingDimension) {
        throw new Error(
          `Query embedding has wrong dimensions: expected ${this.expectedEmbeddingDimension}, got ${embedding.length}`
        );
      }
  ...
  async enhanceSearchQuery(userQuery) {
    try {
      console.log(
        `🔍 Enhancing query: "${userQuery}" using AI semantic analysis`
      );
      ...
      const enhancedQuery = await this.generateEnhancedQueriesAI(
        userQuery,
        searchIntent
      );
      ...
      enhancedQuery.enhancement_confidence = await this.calculateConfidenceAI(
        userQuery,
        enhancedQuery
      );
  ...
  async generateSearchPatterns() {
    try {
      ...
      this.searchPatterns.furniture_objects =
        await this.generateFurnitureObjectsAI();
      ...
      this.searchPatterns.indian_contexts =
        await this.generateIndianContextsAI();

      // Generate style variations using AI analysis
      this.searchPatterns.style_variations =
        await this.generateStyleVariationsAI();

      console.log(
        `✅ Generated ${
          Object.keys(this.searchPatterns).length
  ...
  async generateFurnitureObjectsAI() {
    try {
      ...
      // AI-powered filtering and enhancement
      const enhancedFurniture = await this.enhanceFurnitureWithAI(
        furnitureObjects
      );
  ...
  async generateIndianContextsAI() {
    try {
      ...
      // AI-powered filtering and enhancement
      const enhancedContexts = await this.enhanceIndianContextsWithAI(
        indianContexts
      );
  ...
  async generateStyleVariationsAI() {
    try {
      ...
      // AI-powered filtering and enhancement
      const enhancedVariations = await this.enhanceStyleVariationsWithAI(
        styleVariations
      );
  ...
  calculateRelevanceScore(term, category) {
    ...
    switch (category) {
      case "room_type":
        if (
          termLower.includes("room") ||
          termLower.includes("area") ||
          termLower.includes("space")
        ) {
          relevance += 0.3;
        }
        break;
      case "design_theme":
        if (
          termLower.includes("modern") ||
          termLower.includes("traditional") ||
          termLower.includes("contemporary")
        ) {
          relevance += 0.3;
        }
        break;
      case "furniture":
        if (
          termLower.includes("sofa") ||
          termLower.includes("bed") ||
          termLower.includes("table") ||
          termLower.includes("chair")
        ) {
          relevance += 0.3;
        }
        break;
      case "material":
        if (
          termLower.includes("wood") ||
          termLower.includes("leather") ||
          termLower.includes("fabric") ||
          termLower.includes("metal")
        ) {
          relevance += 0.3;
        }
        break;
      case "color":
        if (
          termLower.includes("white") ||
          termLower.includes("brown") ||
          termLower.includes("beige") ||
          termLower.includes("blue")
        ) {
          relevance += 0.3;
        }
        break;
      case "indian_context":
        if (
          termLower.includes("indian") ||
          termLower.includes("ethnic") ||
          termLower.includes("cultural")
        ) {
          relevance += 0.3;
        }
        break;
  ...
  async detectSearchIntentAI(userQuery) {
    try {
      ...
      for (const word of words) {
        ...
        if (
          this.searchPatterns.room_types.some((room) =>
            room.toLowerCase().includes(word)
          )
        ) {
        ...
        if (
          this.searchPatterns.furniture_objects.some((furniture) =>
            furniture.toLowerCase().includes(word)
          )
        ) {
        ...
        if (
          this.searchPatterns.materials.some((material) =>
        ...
        if (
          this.searchPatterns.colors.some((color) =>
        ...
        if (
          this.searchPatterns.design_themes.some((theme) =>
        ...
        if (
          this.searchPatterns.indian_contexts.some((context) =>
      ...
      const primaryIntent = Object.keys(intentScores).find(
  ...
  async generateEnhancedQueriesAI(userQuery, searchIntent) {
    try {
      ...
      const primarySearch = await this.enhanceForPrimarySearchAI(
        userQuery,
        searchIntent
      );
      const semanticDesc = await this.enhanceForSemanticDescAI(
        userQuery,
        searchIntent
      );
      const objectFocus = await this.enhanceForObjectFocusAI(
        userQuery,
        searchIntent
      );

      // Generate expanded terms using AI patterns
      const expandedTerms = await this.generateExpandedTermsAI(
        userQuery,
        searchIntent
      );
  ...
  async enhanceForPrimarySearchAI(userQuery, searchIntent) {
    ...
    if (searchIntent.intent === "furniture") {
      const furnitureTerms = this.searchPatterns.furniture_objects.filter(
        (furniture) => userQuery.toLowerCase().includes(furniture.toLowerCase())
      );
  ...
  async calculateConfidenceAI(originalQuery, enhancedQuery) {
    try {
      ...
      const hasDetectedElements = Object.values(
        enhancedQuery.enhanced_query.detected_elements || {}
      ).some(
        (val) =>
      ...
      const hasExpandedTerms = Object.values(
        enhancedQuery.expanded_terms || {}
  ...
  async validateEnhancedQueries(enhancedQuery) {
    ...
    for (let i = 0; i < queries.length; i++) {
      try {
        ...
        if (embedding.length !== this.expectedEmbeddingDimension) {
          console.warn(
            `⚠️ Validation warning for query ${i}: ${embedding.length} dimensions`
          );
        } else {
          console.log(
            `✅ Query ${i} validated: ${embedding.length} dimensions`
          );
        }
      } catch (error) {
  ...
  async generateSearchSuggestions(partialQuery, existingData) {
    try {
      ...
      console.log(
        `🔍 Generating suggestions for: "${partialQuery}" using AI patterns`
      );

      // Generate suggestions using AI patterns
      const suggestions = await this.generateSuggestionsFromAIPatterns(
        partialQuery
      );
      await this.validateSuggestions(suggestions);

      console.log(
        `✅ Generated ${suggestions.length} suggestions with AI patterns`
      );
      return suggestions;
    } catch (error) {
  ...
  async validateSuggestions(suggestions) {
    for (let i = 0; i < Math.min(suggestions.length, 3); i++) {
      try {
        ...
        if (embedding.length !== this.expectedEmbeddingDimension) {
          console.warn(
            `⚠️ Suggestion validation warning: "${suggestions[i]}" generated ${embedding.length} dimensions`
          );
        } else {
          console.log(
            `✅ Suggestion "${suggestions[i]}" validated: ${embedding.length} dimensions`
          );
        }
      } catch (error) {
  ...
  /**
   * Perform vector similarity matching using AI patterns
   */
  async calculateVectorSimilarity(query, storedVector) {
    try {
      const queryEmbedding = await this.getQueryEmbedding(query);
      const similarity = this.cosineSimilarity(queryEmbedding, storedVector);

      console.log(
        `🔍 Vector similarity for "${query}": ${similarity.toFixed(4)}`
      );
      return similarity;
    } catch (error) {
  ...
  cosineSimilarity(vectorA, vectorB) {
    if (vectorA.length !== vectorB.length) {
      throw new Error(
        "Vectors must have same dimensions for similarity calculation"
      );
    }
  ...
  async matchQueryAgainstVectors(query, storedVectors, threshold = 0.3) {
    try {
      console.log(
        `🔍 Matching query "${query}" against ${storedVectors.length} stored vectors using AI patterns`
      );
      ...
      console.log(
        `✅ Found ${matches.length} matches above threshold ${threshold}`
      );
      return matches;
    } catch (error) {
  ...
  async detectExactSearchIntentAI(userQuery) {
    try {
      ...
      const hasSpecificTerms =
        this.searchPatterns.room_types.some((term) =>
      ...
      return {
        ...
        confidence: this.calculateExactSearchConfidence(
          userQuery,
          hasExactKeywords,
          hasQuotedTerms,
          hasSpecificTerms
   ...
   */
  calculateExactSearchConfidence(
    userQuery,
    hasExactKeywords,
    hasQuotedTerms,
    hasSpecificTerms
  ...
  async getInstagramStyleSuggestions(partialQuery, userId = null) {
    try {
      ...
      const behaviorSuggestions = this.generateBehaviorSuggestions(
        userId,
        queryLower
      );
      ...
      console.log(
        `📱 Generated ${uniqueSuggestions.length} Instagram-style suggestions for "${partialQuery}"`
      );
  ...
  generateHashtagSuggestions(query) {
    ...
    for (const [category, hashtags] of Object.entries(this.hashtagPatterns)) {
      for (const hashtag of hashtags) {
        ...
        if (
          cleanHashtag.includes(queryLower) ||
          queryLower.includes(cleanHashtag.split("")[0])
        ) {
    ...
    try {
      ...
      if (userBehavior.preferredCategories.length > 5) {
        userBehavior.preferredCategories =
          userBehavior.preferredCategories.slice(0, 5);
      }
