...
// POST /api/images/collection/recreate - Force recreate collection with correct dimensions
router.post("/collection/recreate", async (req, res) => {
  try {
    const qdrantService = await import("../services/qdrantService.js");
    await qdrantService.default.recreateCollection();

    res.json({
      success: true,
      message:
        "Collection recreated successfully with correct dimensions (384)",
      collection_name: "interior_images_description",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
// GET /api/images/collection/info - Get collection information
router.get("/collection/info", async (req, res) => {
  try {
    const qdrantService = await import("../services/qdrantService.js");
    const info = await qdrantService.default.getCollectionInfo();
